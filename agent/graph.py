...
def _parse_verify_json(text: str) -> tuple[bool, str]:
    """Extract {ok, issue} from an LLM reply that may contain prose or fences."""
    # Strip markdown fences if present
    stripped = re.sub(r"```[a-z]*\s*|\s*```", "", text, flags=re.IGNORECASE).strip()
    ...
    if match:
        try:
            ...
            pass
    # Fallback: if the model produced something unparseable, treat as ok=False
